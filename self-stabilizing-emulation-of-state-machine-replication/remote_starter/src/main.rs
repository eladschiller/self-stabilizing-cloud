...
fn run_application_on_remote_computer(node_info: &NodeInfo) -> Child {
    let write_string = commons::arguments::write_string_from_node_id_and_number_of_writers(
        ARGUMENTS.node_infos.len() as i32,
        node_info.node_id,
        ARGUMENTS.number_of_writers,
    );
    let snapshot_string =
        commons::arguments::snapshot_string_from_node_id_and_number_of_snapshotters(
            node_info.node_id,
            ARGUMENTS.number_of_snapshotters,
        );

    let mut command_string = format!(
        "\"cd {}/application/;cargo run {} -- {} {} -l {} -c {:?} {} {} {} {}\"",
        REMOTE_DIRECTORY_NAME,
    ...
    if ARGUMENTS.delta != 0 {
        command_string.push_str(&format!(" -d {}", ARGUMENTS.delta));
    }
